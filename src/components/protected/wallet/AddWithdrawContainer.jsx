...
import { ADD_WITHDRAW_BUTTONS } from '../../../utils/constants';
...
function AddWithdrawContainer() {
  ...
  const isInsufficientFunds = inputValue > cash && action === 'Cash out';
  ...
  if (isAnimating) {
    ...
    content = !isShowed ? (
      <DefaultViewWrapper>
        ...
        <div
          className={`flex  items-center flex-col mt-4 text-gray-700 px-4 h-[170px] ${
            action === 'Add cash' ? 'justify-center' : 'justify-start'
          }`}
        >
          ...
          <div
            className="flex justify-center text-[10px] text-gray-700 text-center italic"
            style={{ display: isInsufficientFunds ? 'block' : 'none' }}
          >
            {cash > 0 ? (
              <p>
